...
def build_final():
    ...
    for i in [x for x in (2**p for p in range(4))]:
        build_command(["--key_size=8", "--threads="+str(i), "--use_cache_memkind_kmem_allocator"], True, "./results/K8_T"+str(i)+"_both.txt")
        build_command(["--key_size=16", "--threads="+str(i), "--use_cache_memkind_kmem_allocator"], True, "./results/K16_T"+str(i)+"_both.txt")


def write_appendix():
    f = open("./results/appendix.txt", "a")
